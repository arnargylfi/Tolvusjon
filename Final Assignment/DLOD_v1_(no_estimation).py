...
persistent_circle = None    # Added for start/stop/reset
save_image_time = None      # Added for start/stop/reset
skip_points_clear = 0       # Added for start/stop/reset
current_window = None
...
def smooth_trajectory(points, alpha=0.9):
    """
    Smooth the trajectory points using an exponential moving average (EMA).
    Args:
        points (list): List of (x, y) tuples representing the trajectory points.
        alpha (float): Smoothing factor, between 0 (no smoothing) and 1 (maximum smoothing).
    Returns:
...
def circle_inters(h_c, k_c, d, h_s, k_s, r_s):
    """
    Find the intersection points of two circles using the quadratic method.    
    Parameters:
    h_c, k_c: float - Center of the smaller circle (C)
    d: float - Radius of the smaller circle (C)
    h_s, k_s: float - Center of the larger circle (S)
    r_s: float - Radius of the larger circle (S)
    Returns:
    List of tuples [(x1, y1), (x2, y2)] representing the intersection points.
    """
...
# Golf ball detection on/off switch
Golf_ball_onoff = 1  # Set to 0 to disable golf ball speed calculation, 1 to enable

# Your existing code...
def post_process(input_image, outputs, pose_result):  # Added "pose_result"
    """Post-process the model output."""
    global club_points, skip_points_clear, persistent_circle, save_image_time, current_window  # Added
    ...
    # Initialize last position variables
    last_x = None
    last_y = None
    last_last_x = None
    last_last_y = None
    ...
    ball_box_width = None  # Initialize ball_box_width to None
        ...
        try:
            if right_shoulder and pose_result.pose_landmarks and len(club_points) > 2:
                ...
                if last_radius_s == 0:
                    ...
                    last_radius_s = math.sqrt(
                        (last_x - right_shoulder[0])**2 + (last_y - right_shoulder[1])**2)
 
                # Calculate intersection points
                print(f"Circle 1 Center: ({last_x}, {last_y}), Radius: {radius_c}")
                print(f"Circle 2 Center: ({right_shoulder[0]}, {right_shoulder[1]}), Radius: {last_radius_s}")
 
                intersect = circle_inters(last_x, last_y, radius_c, right_shoulder[0], right_shoulder[1], last_radius_s)
                ...
                if distp1 > distp2:
                    center_x, center_y = intersect[0]   # New center point x,y
                else:
                    center_x, center_y = intersect[1]   # New center point x,y
    ...
    if len(club_points) > 1400:
        club_points.pop(0)
 
    # Draw a continuous line connecting the club points
    if len(club_points) > 1:
        smoothed_points = smooth_trajectory(club_points)
        #cv2.polylines(live_video_frame, [np.array(club_points, np.int32)], isClosed=False, color=YELLOW, thickness=2)
    ...
    # Compute club speed only if Golf_ball_onoff is enabled and ball_box_width is not None
    ...
    return live_video_frame  # Changed to live_video_frame
...
fps = int(cap.get(cv2.CAP_PROP_FPS))  # Get FPS from video

output_path = "C:/Users/kristinnh/Desktop/combinedcode.mp4"
